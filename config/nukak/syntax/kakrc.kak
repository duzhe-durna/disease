...
add-highlighter shared/kakrc/shell1 region -recurse '\{' '(^|\h)\K%?%sh\{' '\}' ref nu
add-highlighter shared/kakrc/shell2 region -recurse '\(' '(^|\h)\K%?%sh\(' '\)' ref nu
add-highlighter shared/kakrc/shell3 region -recurse '\[' '(^|\h)\K%?%sh\[' '\]' ref nu
add-highlighter shared/kakrc/shell4 region -recurse '<'  '(^|\h)\K%?%sh<'  '>'  ref nu
add-highlighter shared/kakrc/shell5 region -recurse '\{' '(^|\h)\K-?shell-script(-completion|-candidates)?\h+%\{' '\}' ref nu
add-highlighter shared/kakrc/shell6 region -recurse '\(' '(^|\h)\K-?shell-script(-completion|-candidates)?\h+%\(' '\)' ref nu
add-highlighter shared/kakrc/shell7 region -recurse '\[' '(^|\h)\K-?shell-script(-completion|-candidates)?\h+%\[' '\]' ref nu
add-highlighter shared/kakrc/shell8 region -recurse '<'  '(^|\h)\K-?shell-script(-completion|-candidates)?\h+%<'  '>'  ref nu
...
eval %sh{
    ...
    let types: list<string> = [int bool str regex int-list str-list completions line-specs range-specs str-to-str-map]
    let values: list<string> = [default black red green yellow blue magenta cyan white yes no 'false' 'true']
    let static_words = [$keywords $attributes $types $values] | flatten  | str join ' '
    ...
    [
        $'decl str-list kak_static_words ($static_words)'
        $'add-highlighter shared/kakrc/code/keywords regex ($re_op)($keywords | concat)($re_cl) 0:keyword'
        $'add-highlighter shared/kakrc/code/attributes regex ($re_op)($attributes | concat)($re_cl) 0:attribute'
        $'add-highlighter shared/kakrc/code/types regex ($re_op)($types | concat)($re_cl) 0:type'
        $'add-highlighter shared/kakrc/code/values regex ($re_op)($values | concat)($re_cl) 0:value'
    ]
    | str join ';'
}
